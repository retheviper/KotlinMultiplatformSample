...
fun Application.configureRequestValidation() {
    install(RequestValidation) {
        validate<CreateArticleRequest> { article ->
            when {
                article.title.isEmpty() -> ValidationResult.Invalid("Article title is empty.")
                article.content.isEmpty() -> ValidationResult.Invalid("Article content is empty.")
                article.password.isEmpty() -> ValidationResult.Invalid("Article password is empty.")
        ...
        validate<CreateUserRequest> { user ->
            when {
                user.username.isEmpty() -> ValidationResult.Invalid("Username is empty.")
                user.password.isEmpty() -> ValidationResult.Invalid("Password is empty.")
                user.name.isEmpty() -> ValidationResult.Invalid("Name is empty.")
                user.mail.isEmpty() -> ValidationResult.Invalid("Email is empty.")
