...
final class ChatStore: ObservableObject {
    ...
    var webSocket: ChatSocketSession?
    ...
    func toggleReactionInOpenThread(messageId: String, emoji: String) async {
        let rootId = threadRoot?.id
        await toggleReaction(messageId: messageId, emoji: emoji)
        guard let rootId else { return }
        await openThread(messageId: rootId)
    }
