...
次に iPhone simulator 向けにアプリを build します。
...
simulator を boot し、アプリを install して launch します。
...
Xcode を使う場合は `app/iosApp/iosApp.xcodeproj` を開き、`iosApp` scheme で `iPhone 17` などの iPhone Simulator を選択して実行してください。
...
同じターゲットを iPad simulator 向けに build します。
...
Xcode を使う場合は `app/iosApp/iosApp.xcodeproj` を開き、`iosApp` scheme で `iPad Air 13-inch (M3)` などの iPad Simulator を選択して実行してください。
